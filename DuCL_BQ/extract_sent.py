...
# # 将句子对转换为单个句子
# read_data(train_file="./dataset/BQ/BQ.train.data", dev_file="./dataset/BQ/BQ.valid.data", test_file="./dataset/BQ/BQ.test.data", tgt_file="./dataset/BQ/BQ.sent_all.txt")
...
# drop_duplicate(src_file="./dataset/BQ/BQ.sent_all.txt", tgt_sent_file="./dataset/BQ/BQ.sent_unique.txt",
#                tgt_idx_file="./dataset/BQ/BQ.sent_unique_idx.txt")
...
# merge_fn(idx_file="./dataset/BQ/BQ.sent_unique_idx.txt", sent_file="./dataset/BQ/BQ.sent_unique.txt", merge_file="./dataset/BQ/BQ.sent_unique_merge.txt")



# # 从所有的单个句子中抽取一定数量的句子，比如10000，并写入新文件中。
# # 先选出索引，再根据索引来获取相应的数据。
# def idx_record(src_file, tgt_file, num_sent):
#     with open(src_file, "r", encoding="utf8") as read_file, open(tgt_file, "w", encoding="utf8") as write_file:
#         sent_all = read_file.readlines()
#         lens_sent = len(sent_all)
#         idx_list = [idx for idx in range(lens_sent)]
#         idx_list_sample = random.sample(idx_list, num_sent)
#         sent_list_sample = [sent_all[idx] for idx in idx_list_sample]
#         for sent_idx, sent_single in zip(idx_list_sample, sent_list_sample):
#             write_file.writelines(str(sent_idx) + "\t" +sent_single)
#     print(f"随机抽取了{num_sent}条数据，已记录索引值及其对应的数据。")
#
# idx_record(src_file="./dataset/BQ/BQ.sent_unique_merge.txt", tgt_file="./dataset/BQ/BQ.sent_extract.txt", num_sent=10)
...
extract_data(src_file="./dataset/BQ/BQ.sent_unique_merge.txt", tgt_file_sent_id="./dataset/BQ/BQ.sent_id_extract.txt",
             tgt_file_sent="./dataset/BQ/BQ.sent_extract.txt", num_sent=10000)